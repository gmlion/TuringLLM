# Operator: ChatDev — phase-dialogue SOP
...
Produces: `## State done` + `## Return` block with key `answer`. The workspace artefacts (design.md, code.md, test_report.md, documentation.md) are always written under `../../workspace/.chatdev/`.

This operator implements the ChatDev pattern (patterns.md Group 5): four phases — design, coding, testing, documenting — each phase a dialogue between a role pair. Design uses CEO↔CTO without an acceptance gate; coding / testing / documenting pair a specialist with the reviewer and use `operators/evaluate.md` as an acceptance gate. **A failed verdict loops back into the same phase**; the reviewer's feedback is fed into the next attempt. No iteration cap — convergence is the LLM's judgement (consistent across iterative interpreters in this repo).
...
- `../../workspace/.chatdev/program.md` — copy of PROGRAM.md / task content.
...
**Action:** Stage the task body where every role agent can find it by a fixed path, then start the design dialogue.

Write `{{task}}` (with `{{prior_answer}}` prepended as context if non-empty) to `../../workspace/.chatdev/program.md`. Then start the design dialogue (single-cycle: this is a clean-slate emission, no prior MEMORY content to merge with):

    mkdir -p ../../workspace/.chatdev
    # Write the {{task}} content (with prior_answer prepended if non-empty) to ../../workspace/.chatdev/program.md
...
Finish
## Last Action
ChatDev SOP completed. All four phases (design → coding → testing → documenting) have run.
## Result
Documentation artefact written to ../../workspace/.chatdev/documentation.md
## Return
answer: |
$(cat ../../workspace/.chatdev/documentation.md | sed 's/^/  /')
FINEOF
```
