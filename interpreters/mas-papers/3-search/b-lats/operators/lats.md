# Operator: Language Agent Tree Search (MCTS controller)
...
Receives push-args:
  - `{{task}}` — the task body containing the search task. PROGRAM.md content when bootstrap-loaded; the per-item task text when invoked as a library operator by a meta-framework.
  - `{{prior_answer}}` — a prior operator's answer, or empty if none (unused by LATS — search starts from scratch).
...
**Action:** Materialise the task to a scoped file, persist scoped files, write the root node, transition to `selecting`.
...
**Action:** Append `## Lesson` (if non-empty, R59) to `./scoped/lessons-<chosen_child>.md` lazily and append-only (R64, R65). Increment iter_count. Budget check: if exhausted, emit `## No Solution Found` and set state `done` (R61, R63); else transition to `selecting`.
    ...
    ## Return
    answer: |
      No solution found: $NEW_ITER iterations exhausted with $FAILS terminal_fail nodes.
    EXH_EOF
    else
      cat > ./MEMORY.md << SEL_EOF
    ## State
