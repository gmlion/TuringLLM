...
- **No `reflect.md` reuse.** Inter-round nudging is deferred (`reflect.md`'s contract requires a `verdict` that debate has no analogue for).
- **No aggregator dynamic.** Synthesis is the strategy's own inline cycle, not a pushed dynamic — keeps debate inside Group 4 and prevents drift toward MoA-style aggregation.
...
Mixture of Agents — the second Group-4 pattern — is deferred pending per-prompt model selection in the shell. MoA's `propose.md` dynamic is intentionally distinct from this `opine.md` (different access pattern: opine sees prior-round transcripts; propose sees nothing).
